...
- Team-name rendering: `highlight_winner` defaults to `true` — the side strictly ahead on score
  during `IN` takes the `name_leading` colour, and the side winning during `POST` takes
  `name_winner` (both theme `--primary-text-color` by default) and bold. This is intentionally a
  separate colour from the `IN` score cell (`score_leading`, brown) and the live clock/TV badge
  (`live`, indianred) — these no longer stay in visual sync; see `colors.score_leading` /
  `colors.name_leading` / `colors.name_winner` / `colors.live` in the Colors table. Neither side
  ...
  unchanged. The losing/trailing side always stays plain `name_default`/normal-weight; setting
  ...
  plain `name_default` colour, never bold. A `special_teams` entry colours the name `name_special`
  (Material Blue by default) instead — independent of `highlight_winner` and taking priority over
  the leading/winning colour, though it still bolds when that side is actually ahead/winning.
