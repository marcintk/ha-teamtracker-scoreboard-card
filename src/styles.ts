export const CARD_STYLES = `
  ...
  /* grouped-pill treatment mirroring ha-planetary-solar-system-card's .nav
     buttons: translucent fill, 1px divider border, shared edges collapsed via
     a -1px margin (not flex gap, which forces every button's fractional
     font-scaled width to round to a whole device pixel — on a low-DPI
     display that rounding can land unevenly gap-to-gap, e.g. a visibly wider
     gap on one side; the collapsed shared border has no such leftover space
     to divide, so it can't drift), only the outer corners rounded. */
  .slide-btn {
    position: relative;
    box-sizing: border-box;
    cursor: pointer;
